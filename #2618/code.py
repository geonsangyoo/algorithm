...
def distance(n, prev_pick_A, prev_pick_B):
    ...
    next_index = max(prev_pick_A, prev_pick_B) + 1
    #--A
    pick_A = calc_dist(events[n], A_coord) + distance(n+1, next_index, prev_pick_B)
    #--B
    pick_B = calc_dist(events[n], B_coord) + distance(n+1, prev_pick_A, next_index)
    #judge the answer
    if pick_A <= pick_B:
...
def search(n, prev_pick_A, prev_pick_B):
    ...
    next_index = max(prev_pick_A, prev_pick_B) + 1
    #--A
    pick_A = calc_dist(events[n], A_coord) + memo[next_index][prev_pick_B]
    #--B
    pick_B = calc_dist(events[n], B_coord) + memo[prev_pick_A][next_index]
    #judge the answer
    if pick_A <= pick_B:
        print(PATROL_CAR_A)
        search(n+1, next_index, prev_pick_B)
    else:
        print(PATROL_CAR_B)
        search(n+1, prev_pick_A, next_index)
while True:
    line = f.readline()
    if not line:
        break
